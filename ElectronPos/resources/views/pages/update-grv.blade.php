            ...
            <div class="card card-body mx-3 mx-md-4 mt-n6">
                ...
                <div class="card card-plain h-100">
                    <div class="card-header pb-0 p-3">
                        <div class="row">
                            <div class="col-md-8 d-flex align-items-center">
                                <h6 class="mb-3">Update Goods Received Voucher</h6>
                            </div>
                        </div>
                    </div>
                    <div class="card-body p-3">
                        ...
                        @endif
                        <form method="POST" id="submitForm" action="{{ isset($grv) ? route('grv.send-update', $grv->id) : route('submit-grv') }}">
                          @csrf
                          @if(isset($grv))
                            @method('PUT')
                          @endif
                        ...
                        <div class="row">
                            <div class="form-group">
                                ...
                                <select name="supplier_id" class="form-control border border-2 p-2" required>
                                    @foreach ($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ isset($grv) && $grv->supplier_id == $supplier->id ? 'selected' : '' }}>{{ $supplier->supplier_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3 col-md-6">
                                <label class="form-label">Grn Date</label>
                                <input type="date" name="grn_date" class="form-control border border-2 p-2" required value="{{ isset($grv) ? $grv->grn_date : '' }}">
                                ...
                                <select name="payment_method" class="form-control border border-2 p-2" required>
                                    <option value="cash" {{ isset($grv) && $grv->payment_method == 'cash' ? 'selected' : '' }}>Cash</option>
                                    <option value="card" {{ isset($grv) && $grv->payment_method == 'card' ? 'selected' : '' }}>Card</option>
                                    <option value="credit" {{ isset($grv) && $grv->payment_method == 'credit' ? 'selected' : '' }}>Credit</option>
                                </select>                            
                                @error('barcode')
                            ...
                            <div class="col-md-6">
                                ...
                                <textarea type="text" rows="8" name="additional_information" class="form-control border border-2 p-2" required>{{ isset($grv) ? $grv->additional_information : '' }}</textarea>
                            </div>
                            <div class="mb-3 col-md-6">
                                <label class="form-label">Supplier Invoice Number</label>
                                <input type="text" name="supplier_invoicenumber" class="form-control border border-2 p-2" required value="{{ isset($grv) ? $grv->supplier_invoicenumber : '' }}">
                                @error('description')
                                    <p class="text-danger inputerror">{{ $message }}</p>
                                @enderror
                            </div>
                                    ...
                                    <div class="user-cart">
                                        <div class="card">
                                            <table class="table table-striped">
                                                ...
                                                <tbody>
                                                    <!-- Your cart data goes here -->
                                                    <?php foreach ($stocks as $stock): ?>
                                                    <tr>
                                                        <td><?php echo $stock->product_name; ?></td>
                                                        <td><?php echo $stock->measurement; ?></td>
                                                        <td contenteditable="true" class="quantity"><?php echo $stock->quantity; ?></td>
                                                        <td contenteditable="true" class="unit-cost"><?php echo $stock->unit_cost; ?></td>
                                                        <td class="total-cost"><?php echo $stock->total_cost; ?></td>
                                                        <!-- Add more table cells for other stock attributes -->
                                                    </tr>
                                                    <?php endforeach; ?>
                                                </tbody>
                                            </table>

...
$("document").ready(function(){
    ...
    $("#emailPurchaseOrder").on("click", function () {
        ...
        // Send AJAX request to email the purchase order automatically
        $.ajax({
            url: '{{ route("purchase-order.email", $purchaseOrder->id) }}',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                email: '{{ $purchaseOrderEmail }}', // Use the configured purchase order email
            ...
            success: function(response) {
                if (response.dev_mode) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Email Sent!',
                        text: 'Purchase order has been sent successfully to {{ $purchaseOrderEmail }}',
                        confirmButtonColor: '#3085d6'
                    });
                }
            },
